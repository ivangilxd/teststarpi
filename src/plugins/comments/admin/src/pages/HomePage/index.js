...
function save(id,display){
  ...
  fetch('https://accounting.linarys.com/v1/commentsDisplay/',{method: 'POST',
body: JSON.stringify(data)}) 
  .then(res => {console.log(res.json());location.reload();});
}
function watchindicator(id){
  fetch('https://accounting.linarys.com/v1/commentWatched/'+id) 
...
const HomePage = () => {
    ...
    var listItems;
    useEffect(() => {
        fetch('https://accounting.linarys.com/v1/commentsCMS/') 
        .then(res => res.json()) 
        .then(
            res =>{ console.log(res);
            ...
            <Box padding={4}><Avatar src={comment.picture} alt={comment.name} preview /><Typography variant="alpha">{comment.name}</Typography></Box>
                ...
                {comment.consent==1 &&
                    <Button variant="secondary" fullWidth onClick={() =>{setIsVisible(prev => !prev);setActivated(comment.display==0 ? false : true);setText(comment.feedback);setIdcomment(comment.id);watchindicator(comment.id)} } endIcon={<Write/>}>Display comment</Button>
                }
            </Box>
            );
        ...
        fetch('https://accounting.linarys.com/v1/countCommentsDisplay/') 
        .then(res => res.json()) 
        .then(res => setDisplay(res.total));
        fetch('https://accounting.linarys.com/v1/countCommentsWatched/') 
        .then(res => res.json()) 
